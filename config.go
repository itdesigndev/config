...
type Config struct {
	...
	files map[string]*file.File // map of loaded files used for simpler watch api
}
...
func (c *Config) LoadFile(f string) (err error) {
	...
	err = fmt.Errorf("wrong config file extension '%s'", extension)
	return
}
...
func (c *Config) WatchFile(f string, onReloaded func()) error {
	extension := filepath.Ext(f)
	if extension == ".yaml" {
		return c.WatchYaml(f, onReloaded)
	} else if extension == ".json" {
		return c.WatchJson(f, onReloaded)
	}
	return fmt.Errorf("wrong config file extension '%s'", extension)
}
